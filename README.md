...
|6)ERROR HANDLING |Check if the program is executed with the correct number of arguments ``(argc == 2)``. Check if the file exists and can be opened ``(if ((fd = open(file_name, O_RDONLY) > 0)))``.|[``check stuff``](https://github.com/alessiotucci/FdF_42/blob/main/fdf.c#L133C1-L134C1) I accidentally forgot to remove the exit() function |
